...
/**
 * 9. 外部共有アイテム一覧シートを元に、以下のクレンジングを行い別シートに出力します。
 *  1. B列（パス）の先頭「ドライブ」を、L列（ファイル名）から整形した部署名に置換
 *  2. I列（編集者）とJ列（閲覧者）から内部ドメインのアドレスを削除
 *  3. 全列が完全に一致する行を1行にまとめる（重複排除）
 */
function filterAndCleanExternalSharing() {
  filterAndCleanExternalSharing_();
}

/**
 * 8. "アクセス種別が取得不可またはANYONE_WITH_LINKのアイテム"シートと"ドメイン外の編集者・閲覧者"ユーザーシートを結合し、
 * ID(D列)をキーに重複を排除・集約して最終一覧シートに出力します。
 */
function mergeAllExternalUsers() {
  mergeAllExternalUsers_();
}

/**
 * 7. "ドメイン外の編集者_ファイル別集約"シートと"ドメイン外の閲覧者_ファイル別集約"シートをID(D列)で結合し、
 * "ドメイン外の編集者・閲覧者"シートに出力します。
 */
function mergeExternalSummaries() {
  mergeExternalSummaries_();
}

/**
 * 6. "ドメイン外の編集者"と"ドメイン外の閲覧者"シートから、
 * ID(D列)ごとにメールアドレス(I列)をセル内改行で連結して
 * "ドメイン外の編集者_ファイル別集約"シートと"ドメイン外の閲覧者_ファイル別集約"シートに出力します。
 */
function aggregateEmailsById() {
  execAggregateEmailsById_();
}

/**
 * 5. "編集者一覧"と"閲覧者一覧"シートから、特定のドメイン以外のメールアドレスを持つ行を抽出し、
 * "ドメイン外の編集者"と"ドメイン外の閲覧者"シートに出力します。
 */
function extractExternalUsers() {
  execExtractExternalUsers_();
}
/**
 * 4. 共有権限一覧シートから、編集者と閲覧者をそれぞれ1人1行に分割して"編集者一覧"と"閲覧者一覧"シートに出力します。
 */
...
/**
 * 3. 「共有権限一覧」シートからアクセス種別が 'PRIVATE', 'DOMAIN', 'DOMAIN_WITH_LINK' 以外の行を抽出し、
 ...
 * "共有権限一覧"シートに追記します。
 */
function appendSharedPermissions() {
