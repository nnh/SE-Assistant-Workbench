/*
function exportEmailsToFolders() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getActiveSheet();

  // 1. マイドライブに親フォルダを作成（既にある場合は取得）
  const rootFolderName = "Gmail出力_管理番号別";
  let rootFolder;
  ...
  // 2. D2:G9の範囲のデータを取得
  // [ [ID, 添付, 名, 管理番号], ... ] の形式で取得される
  const dataRange = sheet.getRange("D2:G9").getValues();

  // 管理番号ごとのフォルダキャッシュ（同じ番号が複数回出ても一度だけ作成するため）
  const folderCache = {};

  // 3. 各行の処理
  dataRange.forEach((row, index) => {
    const messageId = row[0]; // D列: メールID
    const groupKey = row[3]; // G列: 管理番号

    if (!messageId || !groupKey) return; // 空白行はスキップ

    try {
      // 子フォルダの作成・取得
      if (!folderCache[groupKey]) {
        const subFolders = rootFolder.getFoldersByName(groupKey);
        if (subFolders.hasNext()) {
          folderCache[groupKey] = subFolders.next();
        } else {
          folderCache[groupKey] = rootFolder.createFolder(groupKey);
        }
      }
      const targetFolder = folderCache[groupKey];

      // メールの取得とPDF化
      const message = GmailApp.getMessageById(messageId);
      const subject = message.getSubject().replace(/[/\\?%*:|"<>]/g, "-"); // ファイル名に使えない文字を置換
      ...
      // 一時的にGoogleドキュメントを作成してPDF変換
      const tempDoc = DocumentApp.create(fileName);
      const docBody = tempDoc.getBody();
      docBody.appendParagraph(`Subject: ${message.getSubject()}`);
      docBody.appendParagraph(`From: ${message.getFrom()}`);
      docBody.appendParagraph(`Date: ${message.getDate()}`);
      docBody.appendParagraph(
        "--------------------------------------------------",
      );
      docBody.appendParagraph(message.getPlainBody());
      ...
      console.log(`保存完了: ${fileName} -> フォルダ: ${groupKey}`);
    } catch (e) {
      console.error(`エラー発生 (Row ${index + 2}): ${e.message}`);
    }
  });
...
function extractSpecificRowsByGroup() {
  ...
  // --- 出力処理 ---
  const newSheetName =
    "抽出結果_" + Utilities.formatDate(new Date(), "JST", "MMdd_HHmm");
  ...
  if (outputData.length > 0) {
    exportSheet
      .getRange(2, 1, outputData.length, lastCol)
...
function extractClarivateEmails() {
  ...
  if (results.length > 0) {
    ...
    SpreadsheetApp.getUi().alert(
      results.length -
        1 +
        " 件のメールを抽出しました。D列のIDを使ってPDF化できます。",
    );
  } else {
