# MedDRAの.ascファイル(SOC/PT/LLT、英語版・日本語版)を読み込み、
# SOC-PT-LLTを結合した1つのdata frame(soc_pt_llt)を作成する
library(tidyverse)

# MedDRAフォルダのベースパスと対象バージョンフォルダ配下の.ascファイル一覧を取得する準備
base_dir <- "/Users/mariko/Library/CloudStorage/Box-Box/Stat/Tools/test20260817"
meddra_dir <- file.path(base_dir, "MedDRA")

# MedDRA直下のバージョン別サブフォルダ名の一覧
meddra_subfolders <- list.dirs(meddra_dir, full.names = FALSE, recursive = FALSE)

# 読み込み対象のサブフォルダ（バージョン）を指定
...
llt <- llt_en %>% inner_join(llt_ja, by="llt_code") %>% filter(llt_currency == "Y" & llt_jcurr == "Y") %>% select(-llt_currency, -llt_jcurr)
