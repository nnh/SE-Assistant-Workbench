...
### フェーズ 4: 共有ドライブポリシーの監査

共有ドライブ自体に設定されているセキュリティ制限や運用のポリシーを監査するフェーズです。

- **runSharedDrivePolicyReportGeneration()**
  - 共有ドライブ自体の設定情報（`archiveSharedDrivePoliciesDriveGet_`）および基本権限（`archiveSharedDrivePoliciesPermissions_`）を取得・アーカイブし、最終的にポリシー監査レポートとしてスプレッドシート（`sharedDrivePolicyReportGenerator_`）へ出力します。
...
1. **必須サービス（Advanced Google Services）**
   - Admin Reports API (`AdminReports`)
...
本プロジェクトは TypeScript をベースとしたコンパイル環境（aside環境等）を想定しています。

- **ソースコード（開発環境）**: `src/` 配下の各TypeScriptモジュール
- **ビルド成果物（GAS環境）**: バンドル・コンパイル後のJavaScriptファイル
