...
GetZoomLog <- function(target) {
  ...
  df_list <- logList %>%
    map_if(~ is.data.frame(.x) && nrow(.x) == 0, ~NULL) %>%
    compact()
...
GetNasLog <- function() {
  ...
  res <- df %>%
    filter(Content != "[Malware Remover] Started scanning.") %>%
...
GetGoogleLog <- function() {
  ...
  df <- df %>%
    select("event" = "イベント", "actor" = "アクター", "日付", "description") %>%
    filter(event != "メールログ検索")
  kUserEdit <- c(
...
SetFortiGateLogHeader <- function(target) {
  if (is.data.frame(target)) {
    colNamesValue <- sapply(seq_len(ncol(target)), function(i) {
      vals <- na.omit(target[[i]])
      if (length(vals) == 0) {
        return(NA)
      }
  ...
  colNamesValue[is.na(colNamesValue)] <- paste0("dummy", which(is.na(colNamesValue)))
  colnames(target) <- colNamesValue
  for (i in 1:ncol(target)) {
    target[[i]] <- target[[i]] %>%
      str_remove(str_c(colNamesValue[i], "=")) %>%
      str_remove_all('"')
  }
  return(target)
}
...
GetFortiGateLog <- function() {
  ...
  df <- temp_df %>% filter(!msg %in% kExclude)
  if (nrow(df) == 0) {
    return(df)
  }
