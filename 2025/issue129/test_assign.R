...
assigned_fieldItems <- assigned_target %>%
    map(~ {
        sheet <- .x
        jpname <- sheet$name
...
if (length(assigned_fieldItems) > 0) {
    res <- bind_rows(assigned_fieldItems)
    source("test_getvisitgroups.r")
    visit_groups <- GetVisitGroups(input_json)
    res <- res %>%
        JoinVisitGroups(visit_groups) %>%
        distinct()
    write_csv(res, "/Users/mariko/Library/CloudStorage/Box-Box/Datacenter/Users/ohtsuka/2025/20251107/AML224-FLT3-ITD_assigned.csv")
} else {
    print("assigned 0件")
}
