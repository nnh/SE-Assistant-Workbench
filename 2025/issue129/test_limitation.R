...
limitation_fieldItems <- limitation_target %>% map( ~ {
    sheet <- .x
    jpname <- sheet$name
...
if (length(limitation_fieldItems) > 0) {
    ...
    limitation_result_filtered <- limitation_result %>%
        filter(
            normal_range_less_than_or_equal_to != "" |
            normal_range_greater_than_or_equal_to != "" |
            validate_numericality_less_than_or_equal_to != "" |
    ...
    write_csv(limitation_result_filtered, "/Users/mariko/Library/CloudStorage/Box-Box/Datacenter/Users/ohtsuka/2025/20251107/AML224-FLT3-ITD_limitation.csv")
} else {
    print("limitation 0件")
}
