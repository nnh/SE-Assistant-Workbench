function getCount() {
  ...
  const computerNameCount = Array.from(computerSet)
    ...
    .filter(x => x.length > 1);
  const targetValues = computerNameCount.map(pcinfo => {
    const pcname = Array.from(new Set(pcinfo.map(([, pcname]) => pcname)))[0];
    const ymdlist = pcinfo
      .map(([ymd, _]) => new DateUtils().formatDate(ymd))
      .join(', ');
    const count = pcinfo.length;
    return [pcname, ymdlist, count];
  });
